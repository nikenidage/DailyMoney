...
public class CustomSparkleUpdater : NetSparkleUpdater.SparkleUpdater
{
    ...
    protected override async Task RunDownloadedInstaller(string downloadFilePath)
    {
        ...
        using (FileStream stream = new FileStream(batchFilePath, FileMode.Create, FileAccess.ReadWrite, FileShare.None,
                   4096, true))
        using (StreamWriter
               write = new StreamWriter(stream,
                   new UTF8Encoding(false)) /*new StreamWriter(batchFilePath, false, new UTF8Encoding(false))*/)
        {
            if (isWindows)
            {

                if (DoExtensionsMatch(Path.GetExtension(downloadFilePath), ".zip"))
                {
                    var updateFolder = Path.Combine(Path.GetDirectoryName(Environment.ProcessPath), "Update");
                    UnZip(downloadFilePath, updateFolder);
                    installerCmd = $"move \"{updateFolder}\\*\" \"{Path.GetDirectoryName(Environment.ProcessPath)}\" ";
                }
                else
                {
                    installerCmd += $" -o{Path.GetDirectoryName(Environment.ProcessPath)} -y";
                }
    ...
    private void UnZip(string downloadFilePath, string folder)
    {
        if(!Directory.Exists(folder))
            Directory.CreateDirectory(folder);

        // Open an existing zip file for reading
        ZipStorer zip = ZipStorer.Open(downloadFilePath, FileAccess.Read);
        ...
        {
            zip.ExtractFile(entry, Path.Combine(folder, Path.GetFileName(entry.FilenameInZip)));
        }
        zip.Close();
    }
